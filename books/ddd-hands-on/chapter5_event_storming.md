...
本章でイベントストーミングを通じて作成された図は以下の通りです。この図では、オンライン書店のドメインにおける主要なドメインイベントと、それらを実行するためのプロセスが示されています。また、コンテキストの境界も定義されています。
...
6. **集約 (Aggregates)**
   ...
   :::message
   値オブジェクト (chapter8) 、エンティティ (chapter9)、集約 (chapter10)の詳細や実装はそれぞれの章で説明します。
...
以降「在庫管理コンテキスト (サブドメイン) 」を対象として扱い、ドメイン駆動設計 を探究していきます。
