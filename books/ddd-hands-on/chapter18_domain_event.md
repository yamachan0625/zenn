...
それでは`Pub/Sub`モデルを利用してドメインイベントの基盤を実装していきましょう。ここでは実装をシンプルにするためにメッセージブローカーの代わりとして`Node.js`の`EventEmitter`を利用します。イベントをパブリッシュする側は `EventEmitter` オブジェクトの `emit` メソッドを使用してイベントを発行し、関連するデータを渡すことができます。一方で、イベントをサブスクライブする側は、`once`メソッドを利用して特定のイベント名に対してコールバック関数を登録することで、イベントを受け取ります。このように`EventEmitter`は`Pub/Sub`モデルのパブリッシャーとサブスクライバーの役割を担います。
...
`Prisma`による集約の永続化が成功した後に、`getDomainEvents`で一時的に記録されたドメインイベントを取り出し、`domainEventPublisher.publish(event)`でイベントをパブリッシュしています。その後`clearDomainEvents`を呼び出し、一時的に記録されたドメインイベントをクリアしています。`update`、`delete`メソッドも同様に修正しましょう。
...
ドメインイベントの基本的な実装が完了しました。ドメインイベントは実際にはどのように活用するのでしょうか？以下にドメインイベントの活用方法を 4 つ紹介します。
...
発行されたイベントを他のコンテキストに通知することで、コンテキストの統合を実現することができる。
