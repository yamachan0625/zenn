...
コマンド系アプリケーションサービスが実行時に受け取るパラメータはドメインの進化により、任意のプロパティが追加される可能性があります。任意のプロパティの追加はテストファイルでコンパイルエラーが出ず、悪い意味でテストに影響を与えません。
...
// test
// 実装側で任意のプロパティが追加されてもコンパイルエラーが出ない
...
そこで、`RegisterBookCommand`のプロパティをすべて必須にするために `Required` を利用しています。これにより、`RegisterBookCommand`のプロパティが変更された場合に、あえてエラーを出し追加のプロパティを利用したテストを強制することができます。これはすべてのプロパティを入力して行うテストケースで有用です。
