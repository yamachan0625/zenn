...
実装した値オブジェクトが値の特徴を満たしていることが確認できました。ですが、まだ未完成です。今のままでは変わらず不正な ISBN で `BookId` を生成することが可能です。値オブジェクトの真髄は値にビジネスルールを値に適用できる点にあります。それでは、ビジネスルールを適用していきましょう。(バリデーション、変換のロジックの実装は省略します)
